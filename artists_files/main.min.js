let state={};let noise;const smartphones="screen and (max-width: 480px) and (orientation: portrait), screen and (max-height: 480px) and (orientation: landscape)";function initImageGlitch(){jQuery.fn.noisy=function(opts){opts=jQuery.extend({},jQuery.fn.noisy.defs,opts);const instance=this;const _pt=[{x:0,y:0},{x:0,y:0},{x:0,y:0}];const element=jQuery(this);const rnd1=[Math.random()+1,Math.random()+1,Math.random()+1];const rnd2=[0,0,0];let cnt=0;const arr=[];let loop=null;let t=null;const{rows}=opts;let{ratio}=opts;const{img}=opts;let mshov=false;const{id}=opts;this.run=function(){let i;for(i=0;i<3;i++){if(rnd1[i]>=1){--rnd1[i];rnd2[i]=Math.random()/4+.03}rnd1[i]+=rnd2[i];cnt+=(38-cnt)*.25;_pt[i].x=Math.ceil(Math.sin(rnd1[i]*Math.PI*2)*rnd2[i]*cnt*2);_pt[i].y=0}const num=(Math.abs(_pt[0].x)+Math.abs(_pt[1].x)+Math.abs(_pt[2].x)+8)/4;i=rows;while(i-=1){const _off=Math.sin(i/rows*Math.PI*(Math.random()/8+1))*.8*num*num;arr[i].css({transform:`translateZ(0) translate3d(0, 0, 0) translateX(${_off}px)`,webkitTransform:`translateZ(0) translate3d(0, 0, 0) translateX(${_off}px)`})}};this.go=function(){mshov=true;clearInterval(loop);loop=setInterval(this.run,30)};this.pause=function(){mshov=false;clearInterval(loop);loop=null;for(let i=0;i<rows;i++){arr[i].css({transform:"translateZ(0) translate3d(0, 0, 0)",webkitTransform:"translateZ(0) translate3d(0, 0, 0)"})}};this.updateContainerBounds=function(){const containerWidth=$(id).outerWidth();const containerHeight=$(id).outerHeight();if(ratio<containerHeight/containerWidth){const newHeight=containerWidth*ratio;element.css({height:`${newHeight}px`,padding:`${(containerHeight-newHeight)/2}px 0px`})}else{const newWidth=containerHeight/ratio;element.css({height:`${containerHeight}px`,padding:`0px ${(containerWidth-newWidth)/2}px`})}};this.changeImage=function(newImage,imageName){const containerWidth=$(id).outerWidth();const containerHeight=$(id).outerHeight();const img=new Image;img.onload=function(){ratio=this.height/this.width;for(let i=0;i<rows;i++){element.find("div").eq(i).attr("alt",`Image of clothing: ${imageName} part ${i}`);element.find("div").eq(i).css({backgroundImage:`url(${newImage})`})}if(ratio<containerHeight/containerWidth){const newHeight=containerWidth*ratio;element.css({height:`${newHeight}px`,padding:`${(containerHeight-newHeight)/2}px 0px`})}else{const newWidth=containerHeight/ratio;element.css({height:`${containerHeight}px`,padding:`0px ${(containerWidth-newWidth)/2}px`})}};img.src=newImage};element.css({position:"relative",padding:`0px calc((430px - ${430*ratio}px) / 2)`});for(let i=0;i<rows;i++){const pos=`${i*100/rows}%`;element.append("<div></div>");element.find("div").eq(i).css({backgroundImage:`url(${img})`,backgroundPosition:`0px ${pos=="0%"?"0.2%":pos}`,backgroundSize:"cover",width:"100%",flex:"1",cursor:"pointer",transform:"translateZ(0) translate3d(0, 0, 0)",webkitTransform:"translateZ(0) translate3d(0, 0, 0)"});arr.push(element.find("div").eq(i))}if(opts.auto){t=setInterval(()=>{if(mshov)return;instance.go();setTimeout(instance.pause(),opts.delay/2*Math.random())},opts.delay)}this.initialize=function(){return this};return this.initialize()};jQuery.fn.noisy.defs={rows:0,ratio:1,img:"",auto:false,delay:7e3}}function initModal(){$(".modal-open").on("click",function(e){const $this=$(this);const modal=$($this).data("modal");$(modal).parents(".modal-overlay").addClass("modal-opened");setTimeout(()=>{$(modal).addClass("modal-opened")},350);$(document).on("click",e=>{const target=$(e.target);if($(target).hasClass("modal-overlay")){$(target).find(".modal-modal").each(function(){$(this).removeClass("modal-opened")});setTimeout(()=>{$(target).removeClass("modal-opened")},350)}})});$(".modal-close").on("click",function(e){e.preventDefault();e.stopImmediatePropagation;const $this=$(this);const modal=$($this).data("modal");$(modal).removeClass("modal-opened");setTimeout(()=>{$(modal).parents(".modal-overlay").removeClass("modal-opened")},350)})}function openModal(modal){if(modal.indexOf("#")==-1){modal=`#${modal}`}$(modal).parents(".modal-overlay").addClass("modal-opened");setTimeout(()=>{$(modal).addClass("modal-opened")},350);$(document).on("click",e=>{const target=$(e.target);if($("#model-modal").hasClass("modal-opened"))return;if($(target).hasClass("modal-overlay")){$(target).find(".modal-modal").each(function(){$(this).removeClass("modal-opened")});setTimeout(()=>{$(target).removeClass("modal-opened")},350)}})}function closeModal(modal){if(modal.indexOf("#")==-1){modal=`#${modal}`}$(modal).removeClass("modal-opened");setTimeout(()=>{$(modal).parents(".modal-overlay").removeClass("modal-opened")},350)}function isSafari(){const ua=navigator.userAgent.toLowerCase();if(ua.indexOf("safari")!=-1){if(ua.indexOf("chrome")>-1){return false}return true}}function turnScreenOn(secs){if(isSafari()){$(".vqvlt-content").css("-webkit-animation",`turn-on-safari ${secs}s linear`)}else{$(".vqvlt-content").css("animation",`turn-on ${secs}s linear`)}}function turnScreenOff(secs){if(isSafari()){$(".vqvlt-content").css("-webkit-animation",`turn-off-safari ${secs}s linear forwards`)}else{$(".vqvlt-content").css("animation",`turn-off ${secs}s linear forwards`)}}function preloader(){const img1=new Image;const img2=new Image;const img3=new Image;const img4=new Image;const img5=new Image;const img6=new Image;const img7=new Image;const img8=new Image;const img9=new Image;const img10=new Image;const img11=new Image;const img12=new Image;const img13=new Image;const img14=new Image;img1.src="https://dwvo2npct47gg.cloudfront.net/gifs/question-block-red.gif";img2.src="https://dwvo2npct47gg.cloudfront.net/gifs/pager.gif";img3.src="https://dwvo2npct47gg.cloudfront.net/gifs/home-background.gif";img4.src="https://dwvo2npct47gg.cloudfront.net/gifs/vhs-camera.gif";img5.src="https://dwvo2npct47gg.cloudfront.net/gifs/awgeDVD.gif";img6.src="https://dwvo2npct47gg.cloudfront.net/gifs/testing-home.gif";img7.src="https://dwvo2npct47gg.cloudfront.net/gifs/tv-colors.gif";img8.src="https://dwvo2npct47gg.cloudfront.net/gifs/42021/cap.gif";img9.src="https://dwvo2npct47gg.cloudfront.net/gifs/testing-3.gif";img10.src="https://dwvo2npct47gg.cloudfront.net/gifs/awgeForum.gif";img11.src="https://dwvo2npct47gg.cloudfront.net/gifs/about-background-combine.gif";img12.src="https://dwvo2npct47gg.cloudfront.net/gifs/hats_2.gif";img13.src="https://dwvo2npct47gg.cloudfront.net/images/holiday22/Yellow-Front.png";img14.src="https://dwvo2npct47gg.cloudfront.net/images/holiday22/Red-Front.png"}function shopPreloader(){let images=[];for(let type in shop){for(let item in shop[type]){images=images.concat(shop[type][item].images)}}}function notify(message){$("#notify-message").text(message.toUpperCase());openModal("notify-modal")}function startTextGlitch(){$(".product-glitch-placeholder").addClass("product-glitch")}function stopTextGlitch(){$(".product-glitch-placeholder").removeClass("product-glitch")}function initTerms(){$(".vqvlt-terms").on("click",()=>{newPage("terms")});$(".vqvlt-privacy").on("click",()=>{newPage("privacy")})}function printCredits(){}function yams(){return"Miss you everyday, you will never be forgotten. RIP A$AP YAMS FOREVER."}function isMobileView(){const mq=window.matchMedia(smartphones);return mq.matches}function getParameterByName(name,url){if(!url)url=window.location.href;name=name.replace(/[\[\]]/g,"\\$&");const regex=new RegExp(`[?&]${name}(=([^&#]*)|&|#|$)`);const results=regex.exec(url);if(!results)return null;if(!results[2])return"";return decodeURIComponent(results[2].replace(/\+/g," "))}function pageSwitch(path){const page=(path.replace("/","")||"blank").split("?")[0];switch(page){case"blank":changePage("./pages/landing.html",3.4);break;case"home":changePage("./pages/home.html",3.4);break;case"about":changePage("./pages/about.html",3.4);break;case"contact":changePage("./pages/contact.html",3.4);break;case"shop":changePage("./pages/shop.html",3.4);break;case"metgala":changePage("./pages/shop-metgala.html",3.4);break;case"product":changePage("./pages/product.html",3.4);break;case"videos":changePage("./pages/videos.html",3.4);break;case"pictures":changePage("./pages/pictures.html",3.4);break;case"privacy":changePage("./pages/privacy.html",3.4);break;case"terms":changePage("./pages/terms.html",3.4);break;case"forum":changePage("./pages/forum.html",3.4);break;case"awgeDVD":changePage("./pages/awgeDVD.html",3.4);break;case"awgeDVD-vol-1":changePage("./pages/awgeDVD-vol-1.html",3.4);break;case"awgeDVD-vol-2":changePage("./pages/awgeDVD-vol-2.html",3.4);break;case"awgeDVD-vol-3":changePage("./pages/awgeDVD-vol-3.html",3.4);break;case"media":changePage("./pages/media.html",3.4);break;case"directed":changePage("./pages/directed.html",3.4);break;case"artists":changePage("./pages/artists.html",3.4);break;case"people":changePage("./pages/people.html",3.4);break;case"artist":changePage("./pages/artist.html",3.4);break;case"sweepstakes":changePage("./pages/sweepstakes.html",3.4);break;case"sweepstakes-rules":changePage("./pages/sweepstakes-rules.html",3.4);break;default:changePage("./pages/error.html",3.4);break}}function newPage(path){state.path=path;window.history.pushState(state,null,path);pageSwitch(path)}function initHash(){let url_query_id=getParameterByName("id");if(url_query_id!=null&&window.location.pathname=="/product"){newPage(`product?id=${url_query_id}`)}else{state.path=window.location.pathname;window.history.replaceState(state,null,window.location.pathname);newPage(state.path)}}window.onpopstate=function(event){console.log(event.state);pageSwitch(event.state.path);state=event.state};function landingLoad(){$(document).keypress(e=>{if(e.which==13){if(window.location.pathname=="/"){newPage("home");$("body").append('<audio src="https://dwvo2npct47gg.cloudfront.net/audio/grim.mp3" type="audio/mp3" autoplay loop></audio>')}}});$("#landing-start-button").click(()=>{newPage("home");$("body").append('<audio src="https://dwvo2npct47gg.cloudfront.net/audio/grim.mp3" type="audio/mp3" autoplay loop></audio>')});$("#landing-content").click(()=>{newPage("home");$("body").append('<audio src="https://dwvo2npct47gg.cloudfront.net/audio/grim.mp3" type="audio/mp3" autoplay loop></audio>')})}function homeLoad(){$(".product-button").click(()=>{newPage("shop")});$(".about-button").click(()=>{newPage("about")});$("#home-awgeDVD").click(()=>{newPage("awgeDVD")});$("#pictures-button").click(()=>{newPage("awgeDVD")});$(".contact-button").click(()=>{newPage("contact")});$(".hm-button").click(()=>{window.open("https://humanmade.jp/")});$(".testing-button").click(()=>{window.open("https://tstng.co/")});$(".rallyrace-button").click(()=>{window.open("https://asaprocky.com")});$(".forums-button").click(()=>{newPage("forum")});$(".home-video-gif").click(()=>{newPage("media")});$(".home-video-text").click(()=>{newPage("media")});$("home-navbar-text").click(()=>{newPage("home")});$("#home-video-wrapper-mobile").click(()=>{newPage("media")})}function contactLoad(){$("#contact-send-vqvlt").click(()=>{if($("#contact-email").val()==""||$("#contact-subject").val()==""||$("#contact-message").val()==""){notify("Fill in all fields");return}fetch(`https://judtgvulg8.execute-api.us-east-1.amazonaws.com/prod/email-contact?
      ...
      &message=${encodeURIComponent($("#contact-message").val())}`);$("#contact-email").val("");$("#contact-subject").val("");$("#contact-message").val("");notify("MESSAGE SENT TO ORDER SUPPORT")});autosize($("#contact-email"));autosize($("#contact-subject"));autosize($("#contact-message"))}function productLoad(){shopPreloader();$("#product-option-1").click(()=>{changeShopType(1)});$("#product-option-2").click(()=>{changeShopType(2)});$("#product-option-3").click(()=>{changeShopType(3)});$("#product-option-4").click(()=>{changeShopType(4)});$("#product-option-tstng").click(()=>{window.open("https://tstng.co/store.html")});$("#product-item-decrease").click(()=>{advanceProductShot(-1)});$("#product-item-increase").click(()=>{advanceProductShot(1)});$("#product-buy-button").click(()=>{if(shop[currentType][currentItem.index].sizes==null){if(!isOutOfStock(`${shop[currentType][currentItem.index].baseSKU}-OS`)){addToCart()}}else if(!isOutOfStock(`${shop[currentType][currentItem.index].baseSKU}-${currentItem.size}`)){addToCart()}});$("#cart-checkout").click(()=>{cleanCart();if(Object.keys(getCart()).length==0)return;closeModal("cart-modal");openCheckoutLink()});$(".product-items-item").click(()=>{if(shop[currentType][currentItem.index].camera!=null){loadModel(shop[currentType][currentItem.index].filename,shop[currentType][currentItem.index].camera);openModal("model-modal")}const localPrice=shop[currentType][currentItem.index].price=="Sold Out"?"Sold Out":`$${shop[currentType][currentItem.index].price}`;$("#model-price").text(localPrice)});const img=new Image;img.onload=function(){$("#product-item").html("");noise=$("#product-item").noisy({rows:30,img:resolveImgURL(shop[currentType][1].images[0]),ratio:this.height/this.width,id:"#product-item"});window.addEventListener("resize",()=>{noise.updateContainerBounds()});$("#product-item-mobile").css("background-image",resolveImgURL(shop[currentType][1].images[0]));let urlQuery_id=getParameterByName("id");let shopRouteMap={};for(let item in shop[1]){shopRouteMap[shop[1][item].baseSKU]=item}for(let item in shop[2]){shopRouteMap[shop[2][item].baseSKU]=item}if(urlQuery_id!=null){if(shopRouteMap[urlQuery_id]!=null){setShopProduct(shopRouteMap[urlQuery_id])}else{newPage("shop")}}console.debug("Product loaded:",urlQuery_id)};img.src=resolveImgURL(shop[currentType][1].images[0]);hydrateCart()}function resolveImgURL(name){return name}function messageLoad(){$("#message-order-num").text(orderData.num);$("#checkout-close-button").on("click",()=>{localStorage.cart="";newPage("")})}function videosLoad(){buildVideoPlayer();const id=getParameterByName("id");if(videos[id]!=null){openFullscreenVideo(id)}else{}}function picturesLoad(){return}function directedLoad(){buildDirectedPlayer();const id=getParameterByName("id");if(directed[id]!=null){openDirectedVideo(id)}else{}}function awgeDVDVolLoad(){$("#dvd-video").get(0).load()}function toggleVideoPlay(id){if(!$(id).get(0).paused){!$(id).get(0).pause()}else{!$(id).get(0).play()}}function awgeDVDLoad(){$("#dvd-video").on("click",()=>{newPage("home")});$("#awgedvd-vol-1").on("click",()=>{newPage("awgeDVD-vol-1")});$("#awgedvd-vol-2").on("click",()=>{newPage("awgeDVD-vol-2")});$("#awgedvd-vol-3").on("click",()=>{newPage("awgeDVD-vol-3")})}function mediaLoad(){$("#media-videos").click(()=>{newPage("videos")});$("#media-pictures").click(()=>{newPage("pictures")});$("#media-directed").click(()=>{newPage("directed")})}function aboutLoad(){$("#about-artists").click(()=>{newPage("artists")});$("#about-people").click(()=>{newPage("people")})}function artistsLoad(){$(".about-artists-artist").click(function(){newPage(`artist?id=${$(this).attr("for")}`)})}function artistLoad(){const id=getParameterByName("id");if(id==null||id==""||artists[id]==null){newPage("artists");return}const artist=artists[id];let bios="";artist.bio.forEach(elem=>{bios=`${bios}<li>${elem}</li>`});$("#artist-name").html(artist.name);$("#artist-background").css("background-image",`url('${artist.gif}')`);$("#artist-bio").html(bios);$("#artist-img").css("background-image",`url('${artist.img}')`);$("#artist-img").css("background-position",artist.imgPos)}function loadPage(dir,time){$("#vqvlt-content").load(dir,()=>{$("#vqvlt-tube-border").removeClass("vqvlt-tube-border-mobile");switch(dir){case"./pages/landing.html":$("#vqvlt-tube-border").addClass("vqvlt-tube-border-mobile");landingLoad();break;case"./pages/home.html":homeLoad();break;case"./pages/contact.html":contactLoad();break;case"./pages/shop.html":break;case"./pages/shop-metgala.html":break;case"./pages/product.html":productLoad();break;case"./pages/videos.html":videosLoad();break;case"./pages/about.html":aboutLoad();break;case"./pages/sweepstakes.html":break;case"./pages/sweepstakes-rules.html":break;case"./pages/pictures.html":picturesLoad();break;case"./pages/awgeDVD.html":awgeDVDLoad();break;case"./pages/awgeDVD-vol-1.html":awgeDVDVolLoad();break;case"./pages/awgeDVD-vol-2.html":awgeDVDVolLoad();break;case"./pages/awgeDVD-vol-3.html":awgeDVDVolLoad();break;case"./pages/media.html":mediaLoad();break;case"./pages/directed.html":directedLoad();break;case"./pages/artists.html":artistsLoad();break;case"./pages/artist.html":artistLoad();break;default:break}initModal();turnScreenOn(time);initTerms()})}function changePage(dir,time){turnScreenOff(.35);setTimeout(loadPage,350,dir,time)}function buildDirectedPlayer(){for(let i=1;i<Object.keys(directed).length+1;i++){$("#videos-container").prepend(`\
        ...
        `)}}function convertTime(value){value=Math.floor(value);let minutes=Math.floor(value/60);let seconds=value-minutes*60;minutes.toString();seconds.toString();if(minutes<10){minutes=`0${minutes}`}if(seconds<10){seconds=`0${seconds}`}return`${minutes}:${seconds}`}function openDirectedVideo(id){if(directed[id].isVevo){$($(`#video-listing-${id}`).addClass("animate-turn-off"));setTimeout(()=>{$(".videos-content").prepend('            <div class="videos-fullscreen-container">                <button id="vqvlt-back" class="vqvlt-home" style="">                    <img src="https://dwvo2npct47gg.cloudfront.net/gifs/awge-home.gif" alt="Home button spinning globe">                </button>                <iframe class="videos-fullscreen-video" style="margin:10vh 10vw;border:none;width: 80% !important; height: 80% !important;" src="https://embed.vevo.com?isrc=USRV81700927&autoplay=true" allowfullscreen="true">                </iframe>            </div>            ');$("#vqvlt-back").click(()=>{closeFullscreenVideo()})},425)}else{$($(`#video-listing-${id}`).addClass("animate-turn-off"));setTimeout(()=>{$(".videos-content").prepend(`\
            ...
            `);setTimeout(()=>{$("#videos-fullscreen-video").get(0).play()},100);$("#vqvlt-back").click(()=>{closeFullscreenVideo()})},425)}}function openFullscreenVideo(id){if(videos[id].isVevo){$($(`#video-listing-${id}`).addClass("animate-turn-off"));setTimeout(()=>{$(".videos-content").prepend('            <div class="videos-fullscreen-container">                <div id="vqvlt-back" class="vqvlt-home" style="">                    <img src="https://dwvo2npct47gg.cloudfront.net/gifs/awge-home.gif" alt="Home button spinning globe">                </div>                <iframe class="videos-fullscreen-video" style="margin:10vh 10vw;border:none;width: 80% !important; height: 80% !important;" src="https://embed.vevo.com?isrc=USRV81700927&autoplay=true" allowfullscreen="true">                </iframe>            </div>            ');$("#vqvlt-back").click(()=>{closeFullscreenVideo()})},425)}else{$($(`#video-listing-${id}`).addClass("animate-turn-off"));setTimeout(()=>{$(".videos-content").prepend(`\
            <div class="videos-fullscreen-container">\
                <div id="vqvlt-back" class="vqvlt-home" style="">\
                    <img src="https://dwvo2npct47gg.cloudfront.net/gifs/awge-home.gif" alt="Home button spinning globe">\
                </div>\
